...
interface  IToken is IERC20,ERC721 {
    //event 
     ...
     */
    event AddressFrozen(address indexed _userAddress, bool indexed _isFrozen, address indexed _owner,uint64 _time);
     ...
     */
    function startTime (uint tokenID) external view returns(uint64);

    /**
     * @dev return the end time of the NFT as a UNIX timestamp
     * @param `tokenID` id of the NFT token
     */
    function endTime( uint tokenID) external view returns(uint256);
